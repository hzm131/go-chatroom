...
当一个新的用户上线时，其他已经登录的用户也能获取最新的在线列表
思路1:  当一个用户上线后，服务器马上把维护的onlineUser map整体推送
思路2:  服务器有自己的策略，每隔一定的时间，把维护的onlineUser map整体推送
思路3：
	1.当一个用户A上线，服务器就把A用户的上线信息，推送给所有在线用户
	2.客户端也需要维护一个map，map中记录了他的好友（目前是所有人）map[int]*User
	3.客户端和服务器的通讯通道，要依赖于serverProcessMes协程
*/
